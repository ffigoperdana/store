...
export function AdminProductManager() {
  ...
  function openProductCreator() {
    const drawer = document.getElementById("create-product") as HTMLDetailsElement | null;
    if (!drawer) return;
    drawer.open = true;
    window.requestAnimationFrame(() => {
      drawer.scrollIntoView({ behavior: "smooth", block: "center" });
      drawer.querySelector<HTMLInputElement>("input")?.focus({ preventScroll: true });
    });
  }

  async function pasteInto(setValue: (value: string) => void) {
    try {
      if (!navigator.clipboard?.readText) throw new Error("Clipboard tidak tersedia.");
      const value = await navigator.clipboard.readText();
      if (!value) throw new Error("Clipboard kosong.");
      setValue(value);
      setNotice("Teks dari clipboard ditempelkan.");
      setError("");
    } catch (clipboardError) {
      setError(clipboardError instanceof Error ? `${clipboardError.message} Gunakan Ctrl+V sebagai alternatif.` : "Clipboard tidak dapat diakses. Gunakan Ctrl+V sebagai alternatif.");
    }
  }
  ...
  return <>
    ...
    <section className="admin-section" id="products">
      ...
      <div className="admin-list-toolbar"><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Cari nama, slug, atau kategori…" /><button type="button" onClick={openProductCreator}>+ Tambah produk</button></div>
    ...
    <section className="admin-section" id="new-variant">
      <div className="admin-section-title compact"><div><p className="eyebrow">INVENTORY & FULFILMENT</p><h2>Varian & stok digital</h2></div><p>Langkah 2 setelah produk dibuat: tentukan paket, harga, dan cara pengirimannya.</p></div>
      ...
      <form className="admin-product-form admin-variant-form" onSubmit={createVariant}>
        <div className="wide admin-form-intro"><h3>Buat varian baru</h3><p>Pilih produk induk yang sudah dibuat, lalu isi paket yang benar-benar akan dibeli pelanggan.</p></div>
        ...
        {variantForm.fulfillmentMode === "SINGLE_SHARED" && <><label>Label tombol/akses<input required value={variantForm.sharedDeliveryLabel} onChange={(event) => setVariantForm({ ...variantForm, sharedDeliveryLabel: event.target.value })} placeholder="Buka template Notion" /></label><label>URL atau kode bersama<div className="admin-paste-control"><input required value={variantForm.sharedDeliveryValue} onChange={(event) => setVariantForm({ ...variantForm, sharedDeliveryValue: event.target.value })} placeholder="https://... atau kode akses" /><button type="button" onClick={() => void pasteInto((value) => setVariantForm({ ...variantForm, sharedDeliveryValue: value }))}>Paste</button></div></label></>}
        {variantForm.fulfillmentMode === "UNIQUE_POOL" && <label className="wide">Stok awal—satu URL/kode per baris<div className="admin-paste-control"><textarea required rows={7} value={variantForm.entries} onChange={(event) => setVariantForm({ ...variantForm, entries: event.target.value })} placeholder={"ACCESS-001\nACCESS-002\nhttps://download.example/item-003"} /><button type="button" onClick={() => void pasteInto((value) => setVariantForm({ ...variantForm, entries: value }))}>Paste</button></div></label>}
    ...
    {editingVariant && <div className="admin-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) { setEditingVariant(null); setSharedDeliveryReplacement(""); setInventoryEntries(""); } }}><form className="admin-edit-modal admin-product-form admin-variant-edit" onSubmit={saveVariant}>
      ...
      {editingVariant.fulfillmentMode === "SINGLE_SHARED" && <div className="wide admin-fulfillment-config"><div className="admin-fulfillment-help"><p><strong>Satu akses bersama:</strong> akses yang sama ditampilkan di website dan dapat dikirim ke email pembeli. Demi keamanan, URL/kode yang sudah disimpan tidak pernah dikirim kembali ke browser.</p><p>Status akses: <strong>{editingVariant.sharedDeliveryConfigured ? "Sudah tersimpan dan aktif" : "Belum diatur"}</strong>. {editingVariant.sharedDeliveryConfigured ? "Biarkan kolom pengganti kosong untuk tetap memakai nilai lama." : "Masukkan URL atau kode untuk mengaktifkannya."}</p></div><label>Label tombol/akses<input required value={editingVariant.sharedDeliveryLabel || ""} onChange={(event) => setEditingVariant({ ...editingVariant, sharedDeliveryLabel: event.target.value })} placeholder="Buka template Notion" /></label><label>{editingVariant.sharedDeliveryConfigured ? "Ganti URL atau kode (opsional)" : "URL atau kode akses"}<div className="admin-paste-control"><input required={!editingVariant.sharedDeliveryConfigured} value={sharedDeliveryReplacement} onChange={(event) => setSharedDeliveryReplacement(event.target.value)} placeholder={editingVariant.sharedDeliveryConfigured ? "Kosongkan = pakai URL/kode yang telah tersimpan" : "Masukkan URL atau kode"} autoComplete="off" /><button type="button" onClick={() => void pasteInto(setSharedDeliveryReplacement)}>Paste</button></div></label></div>}
      {editingVariant.fulfillmentMode === "UNIQUE_POOL" && <div className="wide admin-fulfillment-config"><div className="admin-fulfillment-help"><p><strong>Pool akses unik:</strong> satu baris diberikan hanya ke satu pembeli. Tambahkan stok baru di bawah; kode lama tidak ditampilkan kembali.</p><p><strong>{editingVariant.availableInventory}</strong> tersedia · <strong>{editingVariant.reservedInventory}</strong> dipesan · <strong>{editingVariant.deliveredInventory}</strong> terkirim</p></div><label>Tambah URL/kode unik—satu per baris<div className="admin-paste-control"><textarea rows={7} value={inventoryEntries} onChange={(event) => setInventoryEntries(event.target.value)} placeholder={"ACCESS-101\nACCESS-102\nhttps://download.example/item-103"} /><button type="button" onClick={() => void pasteInto(setInventoryEntries)}>Paste</button></div></label></div>}
