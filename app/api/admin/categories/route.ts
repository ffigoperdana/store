...
const schema = z.object({ name: z.string().trim().min(2).max(80), slug: z.string().trim().regex(/^[a-z0-9-]+$/).max(80), description: z.string().max(240).optional(), sortOrder: z.number().int().min(-999).max(999).default(0), hidden: z.boolean().default(false) });
